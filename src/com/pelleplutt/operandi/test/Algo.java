...
public class Algo {
  ...
  public void testCRC32() {
    String test_str = "hello jolly world!";
    String sA;
    sA = 
    "crc32_tab = [\n" +
    ...
    "  crc = crc ^ ~0;\n" +
    "  while (size--) {\n" +
    "    crc = crc32_tab[(crc ^ byte(buf[i++])) & 0xff] ^ (crc >> 8);\n"+
    "  }\n" +
    "  return crc ^ ~0;\n" +
    "}\n" +
    "test_str = '" + test_str+ "';\n" +
    "return crc32(0, test_str, len(test_str));\n";
    ...
    while (size-- > 0) {
      crc = crc32_tab[(int)((crc ^ (byte)(test_str.charAt(i++))) & 0xff)] ^ (crc >>> 8);
    }
    crc = crc ^ ~0;
    assertEquals(crc, Processor.compileAndRun(false, false, sA).i);
  }

  float calc_iters(float x, float y, int max) {
